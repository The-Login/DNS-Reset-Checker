...
# activated attacks (other attacks can be added by appending them to this list)
attacks = ["ip_fragmentation", "edns_removal", "recursive_delegation"]
...
analysis_domain = "domain.example"
...
# manipulate the global response packet based on different attacks
def craft_attack_packet(global_request, global_response):
    ...
    # create domain in mx_queried_domains if it doesn't exist yet
    if tmp_domain not in mx_queried_domains.keys():
        mx_queried_domains[tmp_domain] = {}

        # if the tmp domain matches the test format, set the attack accordingly
        # test format used: [User]@[2 digits version number][2 digits attack number][6 digits domain number].[analysis domain]
        if re.match("^[0-9]{10}$", tmp_application_identifier) != None:
            mx_queried_domains[tmp_domain]["attack"] = int(tmp_application_identifier[2:4], 10)
        else:
            # Specify attack to start with
            mx_queried_domains[tmp_domain]["attack"] = 0
            # For testing all attacks randomly (lots of services only issue one DNS request)
            mx_queried_domains[tmp_domain]["attack"] = random.randint(0, len(attacks) - 1)

    tmp_attack = attacks[mx_queried_domains[tmp_domain]["attack"] % len(attacks)]

    # do attack scenarios based on the value in mx_queried_domains

    # EXAMPLE response manipulation
    if tmp_attack == "0-tid" and tmp_type == 15 and analysis_domain in tmp_name:
        if global_response[DNS].id == 0:
            global_response[DNS].id = 1
        else:
            global_response[DNS].id = 0

        mx_queried_domains[tmp_domain]["attack"] += 1

    elif tmp_attack == "random-tid" and tmp_type == 15 and analysis_domain in tmp_name:
        while True:
    ...
    elif tmp_attack == "one-off-tid" and tmp_type == 15 and analysis_domain in tmp_name:
    ...
    elif tmp_attack == "0-port" and tmp_type == 15 and analysis_domain in tmp_name:
    ...
    elif tmp_attack == "random-port" and tmp_type == 15 and analysis_domain in tmp_name:
        while True:
            tmp_int = randint(0, 65535)
    ...
    elif tmp_attack == "one-off-port" and tmp_type == 15 and analysis_domain in tmp_name:
    ...
    # IP FRAGMENTATION TEST
    elif tmp_attack == "ip_fragmentation" and tmp_type == 15 and analysis_domain in tmp_name:
        ...
        tmp_nonce = "".join(random.choice(string.ascii_lowercase) for _ in range(10))
        for new_records in range(0, number_of_records_to_add):
            mx_record = DNSRRMX()
            mx_record.type = 15
            # low TTL
            mx_record.ttl = ttl_short
            mx_record.preference = 10
            mx_record.rrname = tmp_domain + dot_authoritative_domain_dot
            mx_record.exchange = "a." + tmp_nonce + ".if." + tmp_application_identifier + dot_authoritative_domain

            if an_mx_records == None:
                an_mx_records = mx_record
            else:
                an_mx_records = an_mx_records / mx_record

        global_response[DNS].an = an_mx_records
        global_response[DNS].ancount = number_of_records_to_add
    ...
    # IP FRAGMENTATION: if IP fragmentation works and a DNS name with the following format is queried, reply with an empty response
        ...
        tmp_type == 1
        and analysis_domain in tmp_name
        and re.search(
            "a\.[a-z0-9]{10}\.if\.[0-9]{10}" + re.escape(dot_authoritative_domain_dot),
            tmp_name,
        )
        != None
    ):
    ...
    # EDNS REMOVAL
    elif tmp_attack == "edns_removal" and tmp_type == 15 and analysis_domain in tmp_name:
        ...
        mx_record.preference = 10
        mx_record.rrname = tmp_domain + dot_authoritative_domain_dot
        mx_record.exchange = "a." + "".join(random.choice(string.ascii_lowercase) for _ in range(10)) + ".er." + tmp_application_identifier + dot_authoritative_domain

        global_response[DNS].an = mx_record
        global_response[DNS].ancount = 1
    ...
    # EDNS REMOVAL: if EDNS validation doesn't work and a DNS name with the following format is queried, don't reply anything
        ...
        and analysis_domain in tmp_name
        and re.search(
            "a\.[a-z0-9]{10}\.er\.[0-9]{10}" + re.escape(dot_authoritative_domain_dot),
            tmp_name,
        )
        != None
    ):
    ...
    elif tmp_attack == "recursive_delegation" and tmp_type == 15 and analysis_domain in tmp_name:

        global_response = strip_dns_packet(global_response)

        number_of_records_to_add = number_of_records_to_add_rd
        an_dname_records = None
        an_cname_records = None
        an_mx_records = None
    ...
    # If the request is asking for anything else than an MX record and is not a special request, return an empty response
    elif tmp_type != 15 and analysis_domain in tmp_name:

        # Removing the legitimate answer
        global_response = strip_dns_packet(global_response)
...
def dns_proxy(pkt):
    ...
    global_response = craft_attack_packet(pkt, global_response)

    # Log global response sent back to the resolver
    if logging:
        # Logging attack and packet data
        if pkt[DNS].qd.qtype == 15:
            # print("Attack: {}".format(attacks[(mx_queried_domains[qdomain]["attack"]-1)%len(attacks)]))
            log_entry["attack"] = attacks[(mx_queried_domains[qdomain]["attack"] - 1) % len(attacks)]
            print("---> {}: Testing {}".format(qname, log_entry["attack"]))
            # Decrease the attack counter for testing purposes (this stops the iteration through the attacks)
            # mx_queried_domains[qdomain]["attack"] -= 1
